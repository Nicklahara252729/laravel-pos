...
class ModalChangePassword {
  ...
  constructor() {
    ...
    this.currentPassword = $(`#${attributeName()[0]['currentPassword']}`);
    this.currentPasswordAddon = $(`#${attributeName()[0]['currenPasswordAddonButton']}`);
   ...
   */
  async modalChangePasswordHandler(url) {
  ...
  toggleCurrentPasswordVisibility() {
    ...
    const isPasswordVisible = this.currentPassword.attr('type') === 'password';
    this.currentPassword.attr('type', isPasswordVisible ? 'text' : 'password');
    const eyeIconClass = isPasswordVisible ? 'mdi-eye-off-outline' : 'mdi-eye-outline';
    const eyeIcon = `<i class="mdi ${eyeIconClass} font-size-18 text-muted"></i>`;
    this.currentPasswordAddon.html(eyeIcon);
  }
  ...
  checkPassword() {
    ...
    const validLength = this.newPassword.val().length >= 8 && this.newPasswordConfirmation.val().length >= 8;
