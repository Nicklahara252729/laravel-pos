...
/**
 * import componnet form
 */
import { formInput } from '../form/input.js';

/**
 * Represents a modal input handler for gratuity produk.
 *
 * @class
 */
class ModalInput {
  ...
  /**
   * Handles modal behavior for adding a new gratuity produk.
   */
  modalAddHandler(url) {
    this.formManager.setMethod('POST');
    this.formManager.setAction(url);
    this.formManager.emptyForm();
    this.titleModal.text('Tambah Gratifikasi');
    this.buttonSubmit.text('Simpan');
  }

  /**
   * Handles modal behavior for editing an existing gratuity produk.
   *
   * @param {string} param - The unique identifier of the gratuity produk to be edited.
   */
  async modalEditHandler(param) {
    ...
    this.titleModal.text('Edit Gratifikasi');
