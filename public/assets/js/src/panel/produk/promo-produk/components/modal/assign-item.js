 ...
 */
import { productItemEl, listingItem, listingKategori } from '../item/assign-item.js';
...
class ModalAssignItem {
   ...
   */
  itemList = (itemObj, syaratPromoValue) => {
    const syaratPromo = syaratPromoValue;
    this.modal.on('click', this.buttonSubmit.selector, () => {
      this.container.toggle(true);
      if (syaratPromo === 'item') {
        listingItem(itemObj);
      } else {
        listingKategori(itemObj);
      }
  ...
  async openModalHandler(param) {
    ...
    const itemObj = this.createOptionsObject();
    this.itemList(itemObj, param.syaratPromoValue);
