...
class FormInput extends FormManager {
  constructor(form, modal, submitButton) {
    ...
    this.paymentList = $(`.${attributeName()[0]['paymentList']}`);
    this.paymentListParent = $(`.${attributeName()[0]['paymentListParent']}`);
    this.paymentListGroup = $(`.${attributeName()[0]['paymentListGroup']}`);
  }
  ...
  /**
   * payment list toogle handler
   */
  paymentListToogleHandler() {
    this.paymentList.on('change', this.paymentListParent.selector, function (event) {
      const data = $(this).data('group').replace(/ /g, '-');
      $(`${this.paymentListGroup.selector}-${data}`).toggle(this.checked);
      $(`${this.paymentListGroup.selector}-${data} input[type="checkbox"]`).prop(
        'checked',
        this.checked
      );
    });
